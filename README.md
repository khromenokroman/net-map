...
- Веб-страница с общей сводкой и тремя вкладками: «Карта и хосты» (карта подсети — шлюз в центре, хосты
  вокруг, хосты с одинаковым IP рядом; таблица с поиском, фильтрами и сортировкой), «DHCP» (серверы, клиенты,
  предупреждения; ссылка `…/#dhcp`) и «События» (поиск, фильтры, счётчик новых событий на ярлыке вкладки;
  ссылка `…/#events`). Светлая и тёмная тема, автообновление.
